...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libhoosatwallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libhoosatwallet.UTXO{
	...
	mergeTransactionBytes, err := libhoosatwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by hoosatwallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libhoosatwallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
	...
	unsignedTransactionBytes, err := libhoosatwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) estimateMassAfterSignatures(transaction *serialization.PartiallySignedTransaction) (uint64, error) {
	...
	transactionWithSignatures, err := libhoosatwallet.ExtractTransactionDeserialized(transaction, s.keysFile.ECDSA)
	if err != nil {
		return 0, err
	}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libhoosatwallet.UTXO, requiredAmount uint64) (
	additionalUTXOs []*libhoosatwallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libhoosatwallet.UTXO{
