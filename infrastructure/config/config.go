...
import (
	...
	"github.com/Hoosat-Oy/hoosatd/util/network"
...
const (
	defaultConfigFilename      = "hoosatd.conf"
	...
	sampleConfigFilename    = "sample-hoosatd.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for hoosatd.
	DefaultAppDir = util.AppDir("hoosatd", false)
...
// Flags defines the configuration options for hoosatd.
//
...
type Flags struct {
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in HSAT/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for hoosatd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default hoosatd configuration
...
//
// The above results in hoosatd functioning properly without any config settings
...
// createDefaultConfig copies the file sample-hoosatd.conf to the given destination path,
