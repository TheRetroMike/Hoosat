...
func TestPOW(t *testing.T) {
	...
	testutils.ForAllNets(t, false, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		err = tc.ValidateAndInsertBlock(invalidBlockWrongPOW, true, new(externalapi.DomainHash))
		...
		err = tc.ValidateAndInsertBlock(abovePowMaxBlock, true, new(externalapi.DomainHash))
		...
		err = tc.ValidateAndInsertBlock(negativeTargetBlock, true, new(externalapi.DomainHash))
		...
		if consensusConfig.Name != "hoosat-mainnet" {
			...
			err = tc.ValidateAndInsertBlock(validBlock, true, new(externalapi.DomainHash))
			if err != nil {
				t.Fatal(err)
			}
		}
...
func solveBlockWithWrongPOW(block *externalapi.DomainBlock) *externalapi.DomainBlock {
	...
	for i := uint64(0); i < math.MaxUint64; i++ {
		state.Nonce = i
		if !state.CheckProofOfWork(new(externalapi.DomainHash)) {
...
func TestCheckParentHeadersExist(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		err = tc.ValidateAndInsertBlock(orphanBlock, true, new(externalapi.DomainHash))
		...
		err = tc.ValidateAndInsertBlock(invalidBlock, true, new(externalapi.DomainHash))
		...
		err = tc.ValidateAndInsertBlock(invalidBlockChild, true, new(externalapi.DomainHash))
...
func TestCheckPruningPointViolation(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		err = tc.ValidateAndInsertBlock(blockWithPruningViolation, true, new(externalapi.DomainHash))
...
func TestValidateDifficulty(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		err = tc.BlockValidator().ValidatePruningPointViolationAndProofOfWorkAndDifficulty(stagingArea, blockHash, false, new(externalapi.DomainHash))
